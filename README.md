...
Reusable GitHub Actions workflows (`workflow_call`) for building, scanning, releasing and
deploying containerised services, Helm charts and Terraform modules. A job-for-job port of
the GitLab [`devops/ci-templates`](../ci-templates) library, tailored to the GitHub Actions
execution model.

**27 workflows · 11 shared scripts · 159 inputs · every job containerised.**
...
flowchart TD
    ...
    CHT["chart.yml<br/>docs ∥ lint → build → push"]
...
| `node-build.yml` · `install` | `npm ci --omit=dev` — the production `node_modules` the packaging Dockerfile copies. Uploads `node-modules-production`. |
